...
int read_dmemin(unsigned int mem[], char file[]);
...
void creat_regout_file(char file[], int regs[]);
...
void creat_monitor_file(char file[], unsigned int monitor[MONITORX_SIZE][MONITORY_SIZE]);
void creat_monitor_yuv_file(char file, unsigned int monitor[MONITORX_SIZE][MONITORY_SIZE]);
void creat_cycles_file(char file[], int clk, int inst_counter);
void creat_dmemout_file(char file[], unsigned int mem[]);
void creat_diskout_file(char file[], unsigned int disk[]);
...
int main(int argc, char* argv[])
{
	...
	while (PC != -1)// as long as there ins't HALT command
	{
		...
		if (io_regs[17]==1)
		{
			check_if_disk_free(disk, io_regs, mem); //checks if disk is done
		}
		...
		if (cmd.opcode == 19 || cmd.opcode == 20)
		{
		...
		if ((cmd.opcode == 19||cmd.opcode == 20) && (regs[cmd.rs] + regs[cmd.rt] == 9)) //monitor command
		{
	...
	creat_regout_file(argv[6],regs);
	creat_monitor_file(argv[11],monitor);
//	creat_monitor_yuv_file(argv[12], monitor);
	creat_cycles_file(argv[9], clk, inst_counter);
	creat_dmemout_file(argv[5], mem);
	creat_diskout_file(argv[13], disk);
	fclose(fp_trace);
...
int read_imemin(unsigned int inst[], char file[])//reads the imemin.txt file and puts it in array.
{
...
int read_dmemin(unsigned int mem[], char file[])
{
	...
	int i = 0;
	while (feof(fp_dmemin) == 0 && fgets(line, MAX_MEMORY_LINE, fp_dmemin) !=NULL)
	{
		if (strcmp(line, "\n") == 0 || strcmp(line, "\0") == 0) // ignore the white spaces
			continue;
		mem[i] = strtol(line, NULL, 16); //convert string to to long integer of base 16
		i++;
	}
...
int read_disk(unsigned int disk[], char file[])
{
	...
	while (feof(fp_disk) == 0 && fgets(line, MAX_MEMORY_LINE, fp_disk) !=NULL)
	{
		if (strcmp(line, "\n") == 0 || strcmp(line, "\0") == 0) // ignore the white spaces
			continue;
		disk[i] = strtol(line, NULL, 16); //convert string to to long integer of base 16
		i++;
	}
...
void check_if_disk_free(int io_regs[])
{
	if (io_regs[3] == 1) //timer is trigged. corrent timer value = 1024, disk is done.
	{
		io_regs[14] = 0;
		io_regs[17] = 0;
		io_regs[4] = 1; // disk interaption. disk is done doing is job
...
void monitor_command(int regs[], int io_regs[], unsigned int monitor[MONITORX_SIZE][MONITORY_SIZE], Cmd cmd)
{
	if (cmd.opcode == 20)
	{
		if (io_regs[18] == 1) //out instraction
		{
...
Cmd interrupt_occured(int io_regs[],int inst[], int* PC_ptr, int reti_busy, Cmd cmd)
{
	if (reti_busy == 0) //we are done with the last interapt
	{
		...
		*PC_ptr = io_regs[6]; //the new PC is the one in irqhandler
		...
		return cmd = Line_to_command(new_command); //return command
	}
